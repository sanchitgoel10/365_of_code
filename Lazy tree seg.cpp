...
void updateSegmentTreeLazy(int* tree,int* lazy,int low,int high,int startR,int endR,int currPos,int inc){
	...
	if(lazy[currPos] !=0){                //pehele check kar ki kahi lazy me kuch hai to nhi
		tree[currPos] += lazy[currPos];

		if(low!=high){    //this is check ki leaf node to nhi hai tabhi child hoga 
	...
	if(startR<= low && high <= endR){
		tree[currPos] += inc;       //inc kar aur aage lazy ke child me store kara le
		if(low!=high){
			lazy[2*currPos] += inc;
